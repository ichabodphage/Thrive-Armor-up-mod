    ...
    if (microbeComponent !is null)
        ...
        if(damageType == "toxin"){
            ...
            playSoundWithDistance(world, "Data/Sound/soundeffects/microbe-toxin-damage.ogg", microbeEntity);
		     ...
		     auto compoundId = SimulationParameters::compoundRegistry().getTypeId("steel");
             auto compoundBag = world.GetComponent_CompoundBagComponent(microbeEntity);
             if(compoundBag.getCompoundAmount(compoundId)>=0)
             {
              MicrobeOperations::takeCompound(world, microbeEntity,compoundId, amount);
             }
              if(compoundBag.getCompoundAmount(compoundId)<=0){
